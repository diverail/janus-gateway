...
#define JANUS_RABBITMQEVH_EXCHANGE_TYPE "direct"
...
int janus_rabbitmqevh_connect(void) {
	...
	if(exchange != NULL) {
		...
		amqp_exchange_declare(rmq_conn, rmq_channel, rmq_exchange, amqp_cstring_bytes(exchange_type), 0, 1, 0, 0, amqp_empty_table);
		result = amqp_get_rpc_reply(rmq_conn);
		if(result.reply_type != AMQP_RESPONSE_NORMAL) {
